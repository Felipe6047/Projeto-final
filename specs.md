...
* Todos os botões, links e elementos interativos devem possuir `aria-label` ou texto descritivo claro.
* Inputs com validação devem usar `aria-invalid` (true/false) e `aria-describedby` apontando para a mensagem de erro.
* Foco visível obrigatório em toda a aplicação para navegação por teclado (tecla `Tab`).
* Garantir contraste mínimo AAA para textos sobre fundos coloridos, especialmente botões primários.
...
* **CPF:** Aplica algoritmo real de validação de CPF. Se inválido, o input fica com borda vermelha, `aria-invalid="true"`, e exibe texto de ajuda: *"Por favor, informe um CPF válido."*
...
* **Tratamento de Estados (Loading, Empty, Error):**
  * **Loading:** Telas que buscam dados da API devem exibir componentes Skeletons simulando a estrutura final durante a requisição.
  * **Empty State:** Listas vazias (carrinho, mercado, notificações) devem exibir ilustrações e textos claros indicando a ausência de dados, sugerindo uma ação (Call to Action).
  * **Error State:** Uso consistente de Toasts ou Banners para falhas de validação de API ou ações não autorizadas.
